...
def extract_calibration_constants(mtl_params, satelit, band_num=10):
    ...
    if m_l is None and satelit == "Landsat 7":
        for key, val in mtl_params.items():
            if key.startswith("RADIANCE_MULT_BAND_6"):
                m_l = val
            elif key.startswith("RADIANCE_ADD_BAND_6"):
                a_l = val
            elif key.startswith("K1_CONSTANT_BAND_6"):
                k1 = val
            elif key.startswith("K2_CONSTANT_BAND_6"):
                k2 = val
...
if menu == "🖥️ Dashboard Utama":
    ...
    with st.container(border=True):
        ...
        mtl_file = st.file_uploader("Unggah Berkas MTL Metadata (_MTL.txt) *Opsional*", type=["txt"])
                ...
                try:
                    ...
                    if mtl_file is not None:
                        try:
                            ...
                            band_num = 6 if satelit == "Landsat 7" else 10
                    ...
                    if used_mtl:
                        calibration_detail = f"Kalibrasi Dinamis dari MTL (M_L={M_L:.7f}, A_L={A_L:.5f}, K1={K1:.2f}, K2={K2:.2f})"
                    else:
                        calibration_detail = f"Kalibrasi Default (M_L={M_L:.7f}, A_L={A_L:.5f}, K1={K1:.2f}, K2={K2:.2f})"
    ...
    if st.session_state.lst_results is not None:
        ...
        with st.container(border=True):
            ...
            with dl_col1:
                st.markdown('<div class="card-title" style="margin-bottom: 0px;">🗺️ Peta &amp; Hasil Analisis Spasial LST</div>', unsafe_allow_html=True)
            with dl_col2:
                st.download_button(
                    label="💾 Download Peta LST (.TIF)",
                    data=res['tif_bytes'],
                    file_name="LST_Mapper_Output.tif",
                    mime="image/tiff",
